...
from analytics.persistence.migrations.runner import (
    MigrationRunner,
)
...
def test_discover_orders_migrations_by_numeric_version(
    tmp_path: Path,
) -> None:
...
def test_discover_ignores_invalid_filenames(
    tmp_path: Path,
) -> None:
...
def test_discover_rejects_duplicate_versions(
    ...
    with pytest.raises(
        ValueError,
        match="duplicate migration version: 001",
    ):
        runner.discover()


def test_discover_requires_existing_directory(
    tmp_path: Path,
) -> None:
    connection = MagicMock()
...
def test_runner_rejects_none_connection(
    tmp_path: Path,
) -> None:
    with pytest.raises(
        TypeError,
        match="connection must not be None",
    ):
...
def test_run_applies_pending_migrations(
    ...
    migration_table_cursor = MagicMock()
...
def test_run_uses_existing_applied_versions(
    tmp_path: Path,
) -> None:
    _write_migration(tmp_path, "001_initial.sql")
